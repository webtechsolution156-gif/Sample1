...
import { Smartphone, CheckCircle, XCircle, X } from 'lucide-react';
...
export default function Checkout() {
  ...
  // -----------------------------------------------------------------
  // 3. UPI LINK (with redirect to /orders on success)
  // -----------------------------------------------------------------
  const buildUpiLink = (orderId) => {
    ...
    return `upi://pay?pa=${RECEIVER_UPI_ID}&pn=${encodeURIComponent(
      RECEIVER_NAME
    )}&am=${total.toFixed(2)}&cu=INR&tn=Order%20${orderId}&tr=${orderId}&url=${encodeURIComponent(successUrl)}&ru=${encodeURIComponent(failUrl)}`;
  };
  ...
  // -----------------------------------------------------------------
  // 6. CALLBACK LISTENER – NOW REDIRECTS TO /ORDERS
  // -----------------------------------------------------------------
  useEffect(() => {
    const handleCallback = () => {
      ...
      if (status === 'success' && urlOrderId) {
        ...
        // Redirect to /orders after 2 sec
